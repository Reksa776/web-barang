<x-main title="Barang Keluar" loginFoto="{{$login->foto}}" loginNama="{{$login->nama}}" loginUid="{{$login->uid}}" loginAlamat="{{$login->alamat}}" loginFullName="{{$login->full_name}}" loginTelepon="{{$login->telepon}}" loginAbout="{{$login->About}}" loginEmail="{{$login->Email}}">
          ...
          <div class="col-md-6 col-lg-12">
            <div class="card">
                      ...
                      <button type="button" class="btn btn-block btn-warning" data-toggle="modal" data-target="#edit{{$Barang->id}}">Edit</button>
                        <div class="modal fade" id="edit{{$Barang->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                              ...
                              <form action="/barang_keluar/edit/{{$Barang->id}}" method="post">
                                  {{ csrf_field() }}
                                <div class="modal-body">
                                  <div class="form-group">
                                    <label for="tanggal">Tanggal</label>
                                    <input type="date" class="form-control" id="tanggal" value="{{$Barang->tanggal}}" name="tanggal" placeholder="Tanggal">
                                  </div>
                                  <div class="form-group">
                                    <label for="namaBarang">Nama Barang</label>
                                    <select class="custom-select rounded-0" id="namaBarang" name="namaBarang">
                                  ...
                                  <div class="form-group">
                                    <label for="penerima">Penerima</label>
                                    <input type="text" class="form-control" id="penerima" value="{{$Barang->penerima}}" name="penerima" placeholder="Penerima">
                                  </div>
                                  <div class="form-group">
                                    <label for="keterangan">Keterangan</label>
                        ...
                        <form action="{{url('barang_keluar', $Barang->id)}}" method="post">
                          @method('delete')
                          @csrf
                          <button type="submit" class="btn btn-danger show_confirm">Hapus </button>
                        </form>
                        <a href="/barang_keluar/print/{{$Barang->id}}" class="btn btn-primary" target="_blank">Print</a>
              ...
              <div class="card-footer clearfix">
                ...
                <a href="/barang_keluar/export" target="_blank" class="btn btn-primary">Export Laporan</a>
              </div>
            </div>
        </div>
    </div>
